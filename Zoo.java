...
public class Zoo {
    ...
    private List<Sweemable> sweemmers (){
        List<Sweemable> res = new ArrayList<>();
        for (Animal animal:animals) {
            if (animal instanceof Sweemable) {
                res.add((Sweemable) animal);
            }
        }
        return res;
    }

    public Animal championBySweem () {
        List<Sweemable> start = sweemmers();
        Sweemable champ = start.get(0);
        for (Sweemable sweemmer:start) {
            if (champ.sweemSpeed()< sweemmer.sweemSpeed()){
                champ = sweemmer;
            }
        }
        return (Animal)champ;
